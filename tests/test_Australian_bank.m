...
testcase = set_Australian_bank_testcase();
% Load testcase
...
vertices = ceil(vertices*10^4)/10^4;
rays     = ceil(rays*10^4)/10^4;
result_table = [cell2table(agent2'), array2table(vertices)];
result_table.Properties.VariableNames = {'Agent' 'theta_1' 'theta_2' 'theta_3'};
writetable(result_table, testcase.result_table_vertices_filename, 'delimiter', '\t');

result_table = [cell2table(agent3'), array2table(rays)];
result_table.Properties.VariableNames = {'Agent' 'theta_1' 'theta_2' 'theta_3'};
writetable(result_table, testcase.result_table_rays_filename, 'delimiter', '\t');
